...
**사용자(User)**:
서비스에 고유한 계정을 가지고 자신에게 속한 관심 종목, 알림 조건과 알림 수신 설정을 관리하는 개인 투자자.
_Avoid_: 운영자, 로컬 사용자, 서비스 계정

**외부 로그인 신원(External Identity)**:
카카오 같은 외부 로그인 제공자가 확인했으며 하나의 사용자와 연결되는 신원. 서비스 내부 사용자 및 알림 연결과는 서로 다른 개념이다.

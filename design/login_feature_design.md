...
- 사용자별 시스템 알림 활성화 설정
- 사용자별 맞춤 분석 본문
- 사용자별 알림 조건 평가와 런타임 알림 발송
- 사용자별 `NotificationConnection`
...
`login()`은 사용자 계정을 조회하거나 생성하는 것만으로 끝나지 않는다. 이후 요청에서도 해당 사용자 계정으로 인증되었다고 판단할 수 있는 상태가 성립해야 로그인 완료로 본다. 도메인 협력은 이 상태를 추상화하고, 구현에서는 별도의 로그인 시도와 서비스 세션이 인증 상태를 지원한다.
...
- Electron은 verifier challenge와 함께 5분짜리 로그인 시도를 만들고 기본 브라우저에서 카카오 인증을 시작한다.
- 카카오 콜백이 로그인 시도를 완료하면 Electron은 verifier로 완료 결과를 한 번만 교환한다.
- 서버는 30일 절대 만료의 자체 서비스 세션을 발급하고 원문 대신 토큰 해시를 저장한다.
- Electron main process만 원문 서비스 세션을 다루며 `safeStorage`로 암호화해 보관한다. OS 암호화를 사용할 수 없으면 디스크에 평문으로 저장하지 않는다.
- API 응답에는 최소 계정 정보만 반환하며 카카오 access/refresh token은 저장하거나 노출하지 않는다.
- 로그인 실패·취소·만료 시 사용자 계정 데이터나 알림 연결을 만들지 않고 로그인 화면에서 재시도한다.
...
공유 종목 분석에서 시스템 시장 신호가 감지되면 다음 조건을 모두 만족하는 사용자에게 기본 알림을 자동 발송한다.

- 해당 종목의 관심 종목 구독이 활성 상태다.
- 카카오 알림 연결이 활성 상태다.

초기 구현에서는 별도의 수신 동의 설정이나 사용자 알림 조건이 없어도 기본 알림을 발송한다. 관심 종목 구독과 활성 알림 연결을 기본 알림 수신 의사로 간주한다. 여기서 모든 사용자는 서비스의 전체 사용자 계정이 아니라 위 조건을 만족하는 모든 알림 대상 구독자를 뜻한다.

사용자별 음소거와 신호 종류별 수신 설정은 후속 반복으로 미룬다. 해당 설정이 도입되면 관심 종목 구독과 알림 수신 의사를 분리하되, 초기 구현의 기본 발송 정책을 변경하는 명시적인 정책 결정으로 다룬다.

현재 1차 구현은 공용 분석과 사용자별 구독 접근 경계까지만 완성한다. 사용자별 조건 평가, `NotificationConnection`과 런타임 알림 발송은 아직 수행하지 않으며 후속 설계에서 이 목표 정책을 구체화한다. 서비스 로그인은 `talk_message` 권한을 요청하거나 카카오 access/refresh token을 저장하지 않는다.
...
사용자 알림 조건은 조건마다 독립적으로 평가하며, 하나의 `UserAlertCondition`과 하나의 `StockAnalysis` 조합마다 별도의 `AlertEvaluation`을 생성한다. 한 사용자가 같은 종목에 여러 활성 조건을 가지고 있더라도 평가 결과를 하나로 합치지 않는다. 이를 통해 조건별 판단 근거, 중복 방지와 발송 이력을 독립적으로 추적한다.

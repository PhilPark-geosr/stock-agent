...
서비스는 한 PC의 로컬 프로필이 아니라 여러 사용자가 같은 서버를 이용하는 다중 사용자 서비스로 전환한다. `사용자(User)`는 관심 종목, 사용자 알림 조건과 알림 연결의 소유권 및 접근 제어 경계다.
...
MVP 구현에서는 외부 로그인 제공자로 카카오만 지원한다. 이는 OOA 유스케이스의 목표가 아니라 첫 구현에서 선택한 외부 시스템 제약이다. 서비스 내부 사용자는 외부 제공자의 식별자와 분리된 고유 식별자를 가진다.
...
사용자의 목표는 외부에서 확인된 신원으로 서비스에 로그인하는 것이다. 확인된 신원으로 가입한 내부 사용자가 없으면 최초 로그인 과정에서 사용자를 자동 생성한다.
...
3. 주식 분석 서비스가 확인된 외부 신원으로 이미 가입된 내부 사용자를 찾는다.
4. 기존 연결이 있으면 해당 사용자로 로그인한다.
5. 연결이 없으면 내부 사용자와 외부 로그인 신원 연결을 함께 생성한다.
6. 사용자가 인증된 서비스 사용을 시작한다.

#### 사후 조건

- 성공 시 하나의 외부 로그인 신원은 정확히 하나의 내부 사용자와 연결된다.
- 최초 로그인이라면 내부 사용자와 외부 로그인 신원 연결이 함께 존재한다.
- 로그인만으로 알림 연결을 생성하지 않는다.

### 2. 로그인 도메인 모델

로그인 유스케이스에서 핵심 도메인 객체는 `User`와 `ExternalIdentity`다. 외부 신원 제공자는 외부 행위자이며, 세션·컨트롤러·저장소는 도메인 객체가 아니므로 이 모델에 포함하지 않는다.
...
classDiagram
    ...
    User "1" *-- "1..*" ExternalIdentity : 로그인 신원
```
...
| 도메인 객체 | 책임 |
| --- | --- |
| `User` | 서비스 내부의 데이터 소유권과 사용자 생명주기를 대표 |
| `ExternalIdentity` | 외부 제공자가 확인한 신원을 내부 사용자와 연결 |

#### 불변식

- 같은 제공자의 같은 외부 사용자 식별자는 둘 이상의 `User`에 연결될 수 없다.
- 서비스 데이터는 외부 제공자의 식별자가 아니라 내부 `UserId`를 소유권 기준으로 사용한다.
- 외부 로그인 신원 연결이 이미 존재하면 새 `User`를 생성하지 않는다.
- 최초 로그인에서 `User`와 `ExternalIdentity`는 둘 다 생성되거나 둘 다 생성되지 않아야 한다.
...
sequenceDiagram
    ...
    alt 기존 외부 로그인 신원 연결이 있음
        ...
        Note over System: User와 ExternalIdentity를 함께 생성
    end
...
첫 번째 구현에서는 외부 신원 제공자로 카카오를 선택한다. 아래의 `KakaoIdentityGateway`, 콜백 처리와 세션 발급은 분석 모델이 아니라 이 구현 선택을 실현하는 설계 객체다.

### 1. 카카오 신원 연결 예시

카카오가 `kakao-user-7788`이라는 계정 식별자를 확인해 주었고 서비스에 다음 연결이 저장되어 있다면 `user-42`가 기존 사용자다.

```text
KAKAO + kakao-user-7788 → user-42
```

연결이 없다면 새 내부 사용자와 연결을 함께 만든다.

```text
KAKAO + kakao-user-7788 → user-99
```

이후 같은 카카오 계정으로 로그인하면 `user-99`를 다시 찾는다. 관심 종목과 알림 조건은 내부 `UserId`의 소유이므로 이전 데이터를 그대로 사용한다.

### 2. 설계 클래스 다이어그램 초안

도메인 모델과 SSD의 책임을 구현 객체에 배정한 첫 설계안이다. 세션 전달 방식이 아직 결정되지 않았으므로 관련 인터페이스는 추상적인 책임으로만 둔다.
...
classDiagram
    ...
    class AuthenticateWithKakao {
        <<control>>
        +startLogin()
        +authenticate(callbackResult)
    }

    class User {
        <<entity>>
        +UserId id
        +register(externalIdentity) User
    }
    ...
    class KakaoIdentityGateway {
        <<interface>>
        +createAuthenticationRequest()
        +resolveIdentity(callbackResult)
    }

    class UserRepository {
        <<interface>>
        +findByExternalIdentity(provider, providerSubjectId)
        +add(user)
    }
    ...
    LoginController --> AuthenticateWithKakao
    AuthenticateWithKakao --> KakaoIdentityGateway
    AuthenticateWithKakao --> UserRepository
    AuthenticateWithKakao --> SessionIssuer
    AuthenticateWithKakao ..> User : 생성 또는 조회
    AuthenticateWithKakao ..> ExternalIdentity : 생성 또는 조회
    User "1" *-- "1..*" ExternalIdentity
```

### 3. 설계 객체 상호작용 초안

아래 시퀀스는 SSD가 아니라 설계 클래스 사이의 책임 배정을 검토하기 위한 객체 상호작용 다이어그램이다.

```mermaid
sequenceDiagram
    participant Controller as LoginController
    participant Auth as AuthenticateWithKakao
    participant Kakao as KakaoIdentityGateway
    participant UserRepo as UserRepository
    participant Session as SessionIssuer

    Controller->>Auth: authenticate(callbackResult)
    Auth->>Kakao: resolveIdentity(callbackResult)
    Kakao-->>Auth: ExternalIdentity
    Auth->>UserRepo: findByExternalIdentity(provider, providerSubjectId)

    alt 기존 외부 로그인 신원 연결이 있음
        UserRepo-->>Auth: User
    else 최초 로그인
        UserRepo-->>Auth: 없음
        Auth->>Auth: User.register(ExternalIdentity)
        Auth->>UserRepo: add(User)
    end

    Auth->>Session: issueFor(user)
    Session-->>Auth: 인증 완료 결과
    Auth-->>Controller: 로그인 결과
```

Electron에 인증 완료 상태를 전달하는 구체적인 방식은 로그인 세션 정책과 함께 후속 결정한다.
...
classDiagram
    ...
    User "1" --> "*" NotificationConnection
    User "1" --> "*" WatchlistSubscription
...
| 개념 | 책임 |
| --- | --- |
| `User` | 서비스 내부 사용자와 데이터 소유권의 기준 |
| `ExternalIdentity` | 사용자와 외부 로그인 신원의 연결 |
...
- 로그인 제공자와 서비스 내부 사용자 생명주기를 분리한다.

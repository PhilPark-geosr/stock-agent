...
```mermaid
flowchart TD
    Start[Graph Start] --> Split[Node: split_conditions]
    Split --> HasCustom{custom conditions 있음?}
    HasCustom -->|Yes| Context[Node: build_custom_context]
    HasCustom -->|No| Main[Node: main_analysis]
    Context --> Main
    Main --> End[Graph End]
```

...
```mermaid
flowchart TB
    SCREEN["관심종목 분석 데모 화면 (src/renderer.js)"] --> SIDE["좌측 흐름 안내 (src/components/layout.js)"]
    SCREEN --> MAINVIEW["주요 작업 영역 (src/components/layout.js)"]
...
| `src/renderer.js` | mock 데이터와 선택 상태 관리, 컴포넌트 간 이벤트 연결 |

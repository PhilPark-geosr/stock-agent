...
- 브리핑 저장·조회
- 카카오 등 연동 채널 전달과 재시도
- 관심종목 범위의 생성 시점 스냅샷 저장
- 종목 검색 서비스가 확정한 구조화된 범위를 소비할 수 있는 연동 경계
...
사용자는 본인의 브리핑만 조회한다. 정기 브리핑은 관심종목을 대상으로 하며 생성 시점의 대상을 `BriefingScope(sourceType=WATCHLIST)`로 저장한다.

`@종목` 또는 `#필터`의 검색·후보 선택·종목 ID 확정은 PR #23의 종목 검색 컨텍스트가 담당한다. 브리핑 컨텍스트는 문자열을 해석하지 않고, #23이 확정한 구조화된 종목 ID 집합만 입력받아 scope 스냅샷으로 저장한다.
...
브리핑 서비스가 자연어 문자열에서 종목명이나 필터를 해석하지 않는다. PR #23의 종목 검색 서비스가 확정한 `listed_security_id` 집합을 구조화된 요청으로 전달받는다.
...
`@삼성전자`와 `#반도체`의 검색 결과가 겹치는 경우 중복 제거는 #23의 범위 해석 결과 또는 브리핑 입력 경계에서 `listed_security_id` 기준으로 수행한다. #24는 검색 후보 생성이나 필터 확장을 구현하지 않는다.
...
| #23에서 전달된 범위가 중복 | 입력 경계에서 `listed_security_id` 기준 중복 제거 |
| #23에서 전달된 범위가 지나치게 많음 | 최대 종목 수를 제한하고 범위 축소 오류 반환 |
| #23에서 전달된 종목이 비활성 | 신규 브리핑 분석에서 제외하고 실패 사유 기록 |
...
| 컴포넌트 | 책임 |
| --- | --- |
| `BriefingService` | 범위 스냅샷, 종목 분석, 이전 판단 비교, 정렬, 생성 상태 확정 |
| `BriefingScheduleService` | 거래소 세션에 따른 장전·장후 자동 실행과 사용자 순회 |
| `TradingCalendar` | 휴장일·조기 폐장·시간대·서머타임이 반영된 거래 세션 제공 |
| `MarketDataProvider.fetch_close` | 지정 거래일의 확정 종가 스냅샷 제공 |
| `BriefingRepository` | 생성 lease, 브리핑·scope·항목·실패·전달 상태 저장 |
| `KoreanBriefingRenderer` | 종목별 판단과 변경 이유를 포함한 전달 본문 생성 |
| `BriefingDeliveryService` | 저장 완료된 브리핑의 채널 전달과 독립적인 재시도 상태 관리 |

현재 종목 마스터는 PR #23의 구현 범위이므로 #24는 provider symbol을 임시 식별자로 사용한다. #23 병합 후 저장·API 계약을 `listed_security_id`로 전환하되 위 컴포넌트 책임은 유지한다.
...
- 정기 브리핑의 관심종목 범위가 생성 시점 스냅샷으로 저장된다.
- #23이 제공하는 구조화된 종목 범위를 문자열 재해석 없이 소비할 수 있는 경계가 유지된다.
- 한국과 미국 종목이 각 거래소의 거래일·개장·폐장 시각에 맞춰 별도 브리핑으로 생성된다.

## 15. 1차 구현 결정

- 기존 공개 API 호환성을 위해 사용자 식별자는 인증 연동 전까지 `X-User-Id` 요청 컨텍스트로 전달하고, 생략 시 `default`를 사용한다.
- 종목 마스터가 도입되기 전에는 정규화된 provider symbol을 브리핑 항목의 종목 식별자로 사용한다. 저장된 `resolved_symbols`는 생성 시점 대상 스냅샷 역할을 한다.
- 브리핑 생성과 카카오 전달 상태를 분리한다. 상세 렌더링과 전달은 `BriefingDeliveryService`가 담당하며, 전달 실패는 완성된 브리핑을 변경하지 않는다.
- 카카오 기본 텍스트 템플릿의 표시 제한을 넘는 상세 브리핑은 순서가 표시된 여러 메시지로 분할한다.
- 동일 사용자·거래소·유형·거래일 요청은 생성 lease와 고유키로 중복 분석을 막는다. 운영자 `force` 실행은 같은 ID의 `version`을 증가시키고 다시 전달한다.
- `exchange-calendars` 어댑터가 KRX·NYSE·NASDAQ의 휴장일, 조기 폐장, 시간대와 서머타임을 반영한다.
- 장후 브리핑은 지정 거래일의 일봉이 확인된 경우에만 분석하며 미확정 종가는 retryable 실패로 기록한다.
- 관심종목이 없거나 휴장일이면 브리핑 레코드를 생성하지 않는다.

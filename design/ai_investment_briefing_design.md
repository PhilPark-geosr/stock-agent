...
- 브리핑 생성과 카카오 전달 상태를 분리한다. 분석과 항목 저장이 끝난 뒤 전달을 시도하며, 전달 실패는 `FAILED` 전달 레코드에 남기고 완성된 브리핑은 유지한다.
- 동일 사용자·거래소·유형·거래일 요청은 기존 결과를 반환한다. 실패한 전달은 동일 요청에서 재시도할 수 있다.
- 내장 거래일 정책은 지원 거래소의 주말 실행을 차단한다. 공휴일, 조기 폐장, 서머타임 기반 실행 시각은 후속 거래소 캘린더 어댑터가 담당하도록 경계를 유지한다.

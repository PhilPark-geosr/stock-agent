...
| --- | --- | --- |
| 설계 기준선 | 완료 | `35f48d9` |
| Alembic 마이그레이션 | 완료 | `7532811` |
| 사용자 계정과 로그인 서비스 | 완료 | `ead187a` |
| 로그인 시도와 서비스 세션 | 완료 | `8b2b296`, `5fa5bd9` |
| 카카오 서비스 로그인 | 완료 | `1027a34` |
| 사용자별 관심 종목 구독 | 완료 | `f1aaee5` |
| 사용자별 알림 조건 소유권 | 완료 | `ac99bc6` |
| 공유 분석 경계 보호 | 완료 | `37a446a` |
| Electron 로그인 UI와 세션 복원 | 완료 | `7370e73` |
| 통합 실행 안내와 운영 문서 | 완료 | 본 문서 커밋 |

구현된 1차 범위에서는 사용자별 알림 조건 평가와 런타임 알림 발송을 활성화하지 않는다. 이 기능은 `AlertEvaluation`과 `NotificationConnection` 설계가 확정된 후 별도 반복에서 진행한다.
